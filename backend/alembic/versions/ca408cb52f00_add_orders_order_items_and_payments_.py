...
down_revision: str | None = "d9fef4b110fd"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum(
                "pending", "completed", "cancelled", "failed", name="order_status_enum"
            ),
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_order_items_prompt_id", "order_items", ["prompt_id"], unique=False
    )
    op.create_index(
        "ix_order_items_seller_id", "order_items", ["seller_id"], unique=False
    )
    op.create_table(
        "payments",
        sa.Column("id", sa.UUID(), nullable=False),
        ...
        sa.Column(
            "status",
            sa.Enum(
                "pending", "succeeded", "failed", "refunded", name="payment_status_enum"
            ),
            nullable=False,
        ),
        sa.Column(
