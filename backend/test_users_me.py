...
async def test_flow():
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        ...
        login_buyer = await client.post(
            "/api/auth/login", json={"email": email_buyer, "password": "securepassword"}
        )
        token_buyer = login_buyer.json()["access_token"]

        me_buyer = await client.get(
            "/api/users/me", headers={"Authorization": f"Bearer {token_buyer}"}
        )
        ...
        login_seller = await client.post(
            "/api/auth/login",
            json={"email": email_seller, "password": "securepassword"},
        )
        token_seller = login_seller.json()["access_token"]

        me_seller = await client.get(
            "/api/users/me", headers={"Authorization": f"Bearer {token_seller}"}
        )
